...
- **Log Call** — opens a job: customer name, location (together they become
  the official job name), phone, optional email, call notes, and the call
  date/time (defaults to now, editable for a call logged after the fact).
...
```
jobs:     id, name, customer_name, phone, email, call_notes, admin_notes,
